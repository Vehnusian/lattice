...
<div class="mx-auto max-w-(--max-w-page) px-6 pt-10 pb-24">
	<a
		href="/"
	...
	<div class="mt-6 flex items-center gap-2 font-mono text-[10px] uppercase tracking-wider">
		...
		<span class={model.status === 'published' ? 'text-(--color-accent)' : 'text-(--color-ink-subtle)'}>
			{statusLabel}
		</span>
	</div>
	...
	<div class="mt-16 max-w-(--max-w-prose)">
		{@render body()}
	</div>
</div>
